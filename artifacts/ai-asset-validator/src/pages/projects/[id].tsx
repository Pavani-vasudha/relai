...
import { useGetProject, useGetProjectConfig, useUpsertProjectConfig, useValidateAsset, useListValidations, getGetProjectConfigQueryKey, getListValidationsQueryKey, getGetProjectQueryKey } from "@workspace/api-client-react";
...
import { CheckCircle2, XCircle, Clock, Zap, DollarSign, ListChecks, FileText, Upload, Check, AlertTriangle, Fingerprint, Image as ImageIcon } from "lucide-react";
      ...
      <div className="p-8 flex-1">
        <Tabs defaultValue="dashboard" className="w-full">
          ...
          <TabsContent value="validations" className="m-0">
            <ValidationsTab projectId={projectId} />
          </TabsContent>
...
function DashboardTab({ projectId }: { projectId: number }) {
  ...
  // Since useGetProject returns ProjectWithStats (or we assume it does based on data types)
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        setAssetContent(event.target.result as string);
      }
    };
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Rules Override (Optional)</Label>
              <Textarea 
            ...
            <Button 
              className="w-full" 
              onClick={onSubmit}
              disabled={validateAsset.isPending || !assetContent}
            >
      ...
      <div className="space-y-6">
        {result ? (
          <Card className="overflow-hidden">
            <div className={`p-4 flex items-center justify-between border-b ${result.status === 'PASS' ? 'bg-emerald-50/50 dark:bg-emerald-950/20' : 'bg-destructive/5'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className={`text-xl font-bold ${result.status === 'PASS' ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}>
                    {result.status}
                  </h3>
          ...
          <div className="h-full min-h-[400px] border rounded-xl bg-card flex flex-col items-center justify-center text-muted-foreground p-8 text-center border-dashed">
            ...
            <p className="text-sm max-w-sm">
              Configure your asset and click "Run Validation" to see the detailed evaluation results here.
            </p>
          </div>
        )}
      </div>
...
function ValidationsTab({ projectId }: { projectId: number }) {
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Fast, cheap heuristic checks to run before sending to AI.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label>Duplication Check</Label>
                <div className="text-xs text-muted-foreground">Reject identical assets</div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <div className="text-xs text-muted-foreground">Reject if personal info found</div>
              </div>
              <Switch checked={piiCheck} onCheckedChange={setPiiCheck} />
            </div>
            
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label>Blur Check</Label>
                <div className="text-xs text-muted-foreground">Reject blurry images (fast)</div>
              </div>
              <Switch checked={blurCheck} onCheckedChange={setBlurCheck} />
            </div>
          </CardContent>
        </Card>
